...
def fill_miss_by_corr(data, attr, miss):
    ...
    arr.sort()
    missing.sort()
    for i in range(1677787):
        if pd.isnull(data.iloc[i][miss]):
            r = rank(arr, data.iloc[i][attr]) / len(arr)
            if r == 0:
                data.loc[i:i, (miss)] = missing[len(missing) - 1]
            else:
                data.loc[i:i, (miss)] = missing[(int)((1-r)*len(missing))]

# 读取数据
data = pd.read_csv('Chicago Building Violations\\building-violations.csv', sep=',')
violation = nominal_attr_statistic(data, 'VIOLATION STATUS')
nominal_barchart(violation, 'VIOLATION STATUS', 0)

# 五数描述
SSA = quantity_attr_statistic(data, 'SSA')
...
# 使用最高频率填补缺失值
fill_miss_by_freq(data, 'SSA')
fill_miss_by_freq(data, 'VIOLATION LOCATION')
...
fill_miss_by_corr(data, 'Community Areas', 'SSA')
histogram(data, 'SSA')
